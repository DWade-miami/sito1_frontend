...
export default {
    ...
    created () {
        ...
        eventBus.$on('inizia_nuova', (i) => {                           //ricevo al momento in cui decide se rigiocare (si o no), e azzero tutto, pronto per una prossima partita
            if (i == true) {
                ...
                axios.get("https://tris5.herokuapp.com/api/reset",            //azzero le mosse salvate nel backend (mosse che ha fatto l'utente nella partita)
                {}
                ).then().catch(err => {
                    console.log(err)
                })
    ...
    methods : {
        check(a) {
            if (this.q[a] == "-") {
                axios.post("https://tris5.herokuapp.com/api/mossa",       //chiamata backend ogni mossa dell'utente
                {
                T: this.q,
                Index: a,
